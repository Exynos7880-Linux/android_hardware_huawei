...
DevicesFactory::DevicesFactory() : mV2_0DevicesFactory(nullptr), mAOSPDevicesFactory(nullptr) {
    mV2_0DevicesFactory = V2_0::IDevicesFactory::getService().get();
    mAOSPDevicesFactory = new AOSPDevicesFactory();
}

DevicesFactory::~DevicesFactory() {
    if (mAOSPDevicesFactory) {
        delete mAOSPDevicesFactory;
    }
}
...
Return<void> DevicesFactory::openDevice(const hidl_string& device, openDevice_cb _hidl_cb) {
    if (device == AUDIO_HARDWARE_MODULE_ID_PRIMARY) {
        V2_0::IDevice* V2_0PrimaryDevice = openDevice(device);
...
Return<void> DevicesFactory::openPrimaryDevice(openPrimaryDevice_cb _hidl_cb) {
    V2_0::IDevice* V2_0PrimaryDevice = openDevice(AUDIO_HARDWARE_MODULE_ID_PRIMARY);
...
static V2_0::IDevicesFactory::Device hwModuleToV2_0Device(const std::string& name) {
    ...
    if (name == AUDIO_HARDWARE_MODULE_ID_REMOTE_SUBMIX) return V2_0::IDevicesFactory::Device::R_SUBMIX;
    if (name == AUDIO_HARDWARE_MODULE_ID_STUB) return V2_0::IDevicesFactory::Device::STUB;

    return V2_0::IDevicesFactory::Device::PRIMARY;
}

V2_0::IDevice *DevicesFactory::openDevice(const std::string& name) {
    V2_0::Result result;
    ...
    if (mV2_0DevicesFactory) {
        auto ret = mV2_0DevicesFactory->openDevice(
            deviceType,
            [&result, &device](V2_0::Result ret, sp<V2_0::IDevice> dev) {
                result = ret;
                device = dev;
            }
        );
